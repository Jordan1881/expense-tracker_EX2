...
describe("Manage Categories panel", () => {
  ...
  it("opens a slide-over listing seeded categories", async () => {
    ...
    expect(
      within(dialog).queryByRole("button", { name: /delete other/i }),
    ).not.toBeInTheDocument();
    expect(
      within(dialog).queryByRole("button", { name: /rename other/i }),
    ).not.toBeInTheDocument();
  });
  ...
  it("shows an error when create is rejected", async () => {
    fetchMock.mockImplementation(
      async (input: RequestInfo | URL, init?: RequestInit) => {
        const url = String(input);
        const method = (init?.method ?? "GET").toUpperCase();
  ...
  it("deletes a category after confirmation and hides delete for Other", async () => {
    const categories = [
      ...seedCategories,
